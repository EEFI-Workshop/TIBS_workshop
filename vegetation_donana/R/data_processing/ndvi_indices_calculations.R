...
# Load functions
source(here("R","functions","ndvi_calculation_functions.R"))

# Read and process the NDVI data
df_ndvi <- read.csv(here("data","raw","df.ndvi.csv"))
...
saveRDS(all_processed_data, file = here("vegetation_donana", "data","processed","ndvi_processed.rds"))
cat("ndvi_trends_plots.pdf figure is created.", "\n")
# Create plot with transparent non-significant trends
pdf(file = here("vegetation_donana", "data","processed","ndvi_trends_plots.pdf"), height = 12, width = 15)
...
pdf(file = here("vegetation_donana", "data","processed","changes_integrated_ndvi.pdf"), height = 10, width = 15)
...
write.csv(ndvi_metrics, file = here("vegetation_donana", "data","processed","ndvi_metrics.csv"))
