...
get_future_clim = function(coords, scenario, output, years){
  ...
  for(yearx in years){
    ...
    clim = 
      ...
      mutate(ID = coords$ID)%>%
      st_drop_geometry()%>%
      pivot_longer(cols = -c(ID, any_of("Elevation")), names_to = "name", values_to = "value")%>%
...
get_all_futures = function(scenarios, location_type = "plots") {
  future_climate = map_df(scenarios, function(scenario) {
    # Create pattern that specifically matches the location type
    file_pattern = paste0("future_climate_", scenario, "_", location_type, "_\\d{4}\\.csv$")
    ...
    future =
      ...
      map(function(file) {
        ...
        # Remove Elevation column if it exists to standardize columns
        df <- df %>% select(-any_of("Elevation"))
        return(df)
      }) %>%
      bind_rows() %>%
  ...
  # Write combined results to CSV with location-specific name
  write.csv(future_climate,
            file = here("vegetation_donana", paste0("future_climate_all_", location_type, ".csv")),
            row.names = FALSE)
...
train_gam <- function(data, ndvi_metric, bioclim_vars) {
  data$plot <- as.factor(data$plot)
  # Global smooths (shared across all plots) + plot-specific intercepts
  smooths_global <- paste(paste("s(", bioclim_vars, ")", sep = ""), collapse = " + ")
  formula <- as.formula(paste(ndvi_metric, "~ plot +", smooths_global))
...
run_models_scenarios <- function(base_data, scenario_data_list, ndvi_metrics) {
  ...
  for (metric in ndvi_metrics) {
    ...
    for (bioclim_set in get_bioclim_combinations()) {
      ...
      # Train models (GAM excluded - does not work well with this data structure)
      models <- list()
      tryCatch({
